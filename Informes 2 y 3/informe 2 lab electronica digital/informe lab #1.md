...
> Se propone la medidición experimental de la velocidad del sonido en el aire. Esto se logra midiendo el tiempo que tarda en recorrer una distancia determinada una onda ultrasónica que se reflejará en una superficie plana y regresará para ser medida con un sensor de eco. La ecuación relevante es:

v = 2d/t;    t = t_final − t_inicial

donde:
- d = distancia al obstáculo  
- t = tiempo de ida y vuelta  

Midiendo el tiempo total del recorrido se puede calcular la velocidad del sonido 

>El sensor ideal para este experimento es el: HC-SR04 (sensor ultrasónico)
>Los pines necesarios y sus conexiones son:
...
- Se envía un pulso de 10 μs al pin TRIG (se usaría la función micros).
...
>El pin ECHO se configuraría para generar una interrupción CHANGE qur registrará los cambios entre HIGH y LOW, permitiendo medir con precisión el tiempo de ida y vuelta de la onda ultrasónica.

>Para el análisis, el resultado experimental se compararía con el valor teórico de la velocidad del sonido en la temperatura ambiente de Bogotá, que se puede calcular con la ecuación:

v ≈ 331 + 0.6T, donde T es la temperatura en °C.

>Las ventajas principales al usar este montaje de medición son: disminuir el error del resultado al eliminar el error de reacción humano; obtener más precisión en la medición al medir el tiempo en microsegundos, lo cual es imposible de medir manualmente; por último, ofrece la posibilidad de repetir multiples veces el experimento exacto con el fin de hallar un promedio estadístico que le proporcionaría más veracidad al resultado obtenido.