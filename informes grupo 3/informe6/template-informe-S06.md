...
> [En la Actividad 1 la cadencia de muestreo fue muy regular y cercana al valor nominal de 500 ms establecido en el programa. Esto puede verificarse directamente en los timestamps observados en el archivo CSV del Serial Monitor: 500 ms, 1000 ms, 1500 ms, 2000 ms y 2500 ms. La diferencia entre muestras consecutivas es constante e igual a 500 ms, lo que indica que el sistema realizaba el muestreo de forma periódica y estable. En la Actividad 3 se añadieron nuevas tareas al sistema, específicamente la actualización del display OLED mediante comunicación I2C y la lectura del botón con debouncing. A pesar de esto, el muestreo continuó funcionando de manera prácticamente regular y el CSV siguió transmitiéndose sin interrupciones visibles durante el cambio de pantallas. Esto demuestra que el manejo adicional del OLED y del botón no afectó significativamente la periodicidad del sistema. La razón principal es que el programa fue implementado usando temporización basada en millis() y no con delay(). De esta manera, el muestreo, la actualización del OLED y la lectura del botón se ejecutan como tareas independientes dentro del loop() principal sin bloquear la ejecución del resto del código. Además, el display.display() solo se ejecuta una vez por ciclo de muestreo, reduciendo el tiempo ocupado por la comunicación I2C. Aunque el refresco del OLED introduce una pequeña carga adicional de procesamiento, esta sobrecarga es muy baja comparada con el periodo de 500 ms del muestreo. Por ello, en la práctica solo podrían aparecer pequeñas variaciones de algunos milisegundos, pero no una pérdida importante de regularidad.]
...
> [Para que las estadísticas no se calculen desde el inicio de la operación sino únicamente sobre las últimas N muestras, el sistema podría modificarse utilizando una ventana deslizante. En lugar de almacenar sumas acumuladas de todos los datos adquiridos, el programa conservaría solamente las N muestras más recientes y eliminaría automáticamente las más antiguas a medida que llegan nuevas lecturas. Esto permitiría que el promedio, el mínimo y el máximo representen el comportamiento reciente de los sensores y no el historial completo del sistema. La estructura de datos más adecuada para esta implementación es un buffer circular. Este puede construirse con un arreglo bidimensional del tipo int buffer[N][3], donde cada fila representa una muestra completa y las tres columnas corresponden a los valores raw del LDR, LM35 y potenciómetro. Un índice circular indicaría la posición donde debe almacenarse la siguiente muestra. Cuando el índice alcanza el final del arreglo, vuelve al inicio y sobrescribe los datos más antiguos. Así, la memoria utilizada permanece constante independientemente del tiempo de ejecución del sistema. El promedio podría calcularse manteniendo una suma parcial de las muestras presentes en la ventana, restando la muestra antigua y agregando la nueva cada vez que se actualiza el buffer. Para el mínimo y el máximo existen dos posibilidades: recalcularlos recorriendo todas las N muestras en cada actualización, lo cual es más simple, o implementar estructuras adicionales más eficientes, aunque esto aumentaría la complejidad del programa. La principal restricción de esta propuesta es la SRAM limitada del Arduino Uno, que dispone únicamente de 2048 bytes. Aunque cada lectura ADC es de 10 bits, en Arduino normalmente se almacena en variables tipo int, que ocupan 2 bytes. Como cada muestra contiene tres canales, cada muestra requiere aproximadamente 6 bytes de memoria. Por tanto, si toda la SRAM estuviera disponible, el tamaño máximo teórico sería de aproximadamente 341 muestras. Sin embargo, ese límite no puede alcanzarse en la práctica porque parte de la memoria ya está ocupada por otros elementos del programa. El buffer del display OLED SSD1306 consume alrededor de 512 bytes, además de la memoria utilizada por variables globales, buffers seriales, el stack y variables locales. Debido a esto, la memoria realmente disponible para almacenar muestras es considerablemente menor. En condiciones realistas, una ventana segura estaría alrededor de 100 a 200 muestras. Un valor mayor podría provocar inestabilidad o agotamiento de memoria SRAM en el microcontrolador.]